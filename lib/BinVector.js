class BinVector {
    ...
    isNull() {
        return (
            this.vector.reduce(function (acc, bit) {
                return acc + bit;
            }, 0) === 0
        );
    }
    ...
    GF2 = {
        ...
        prod: (binVector) => {
            return new BinVector(binVector).vector
                .reverse()
                .reduce((acc, bit, i) => {
                    return bit === 1 ? acc.GF2.add(this.shiftLeft(i)) : acc;
                }, BinVector.zero(1));
        },
        deg: () =>
        ...
        div: (binVector) => {
            ...
            const { q, r } = (function rec(remainder, quotient) {
                ...
                if (degA >= degB) {
                    ...
                    quotient = quotient.GF2.add(Q);
                    return R.isNull()
                        ? { q: quotient, r: BinVector.zero(1) }
                        : rec(R, quotient);
                } else {
